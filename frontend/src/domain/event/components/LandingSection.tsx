...
const LandingSection: FC<Props> = ({
  ...
  if (!hasImages) {
    return (
      <section className="w-full bg-gradient-to-b from-[#f5ede4] to-[#e8ddd0] px-6 py-10 text-center">
        {groomName && brideName && (
          <p className="text-lg tracking-[0.3em] font-light mb-3 text-gray-600">
            {groomName} <span className="text-gray-400 mx-2">&</span> {brideName}
          </p>
        )}
        <h1 className="text-2xl font-semibold mb-4 leading-relaxed text-gray-800">
          {title}
        </h1>
