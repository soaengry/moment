...
const MyLikesPage: FC = () => {
  ...
  const { items: posts, hasMore, isLoading, loadMore, reset, setItems: setPosts } =
    usePaginatedPosts<PostResponse>(feedApi.getMyLikes, "좋아요한 게시글을 불러오지 못했습니다.");
    ...
    <div className="max-w-lg mx-auto min-h-screen bg-[#faf9f6]">
      <header className={`sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-gray-50 transition-transform duration-300 ${headerVisible ? "translate-y-0" : "-translate-y-full"}`}>
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <h1 className="text-base font-semibold text-gray-800">좋아요한 게시글</h1>
        </div>
      </header>
      ...
      {posts.length === 0 && !isLoading && (
        <div className="text-center py-20">
          <p className="text-sm text-gray-300">좋아요한 게시글이 없습니다</p>
        </div>
      )}
